...
from data import ValidDataset, TrainDataset, TestDataset
test_data_eval = None
test_transform = None
cached_type = None
...
def eval_model(modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True, model=None, train_embed=None, head_layer=8, density=GaussianDensityTorch()):
    ...
    test_data_eval = ValidDataset(data=defect_type, transform=test_transform)
    ...
    labels = []
    embeds = []
    # print("valid set: ", len(dataloader_test))
    with torch.no_grad():
    ...
    return roc_auc, len(dataloader_test)

def test_model(modelname, defect_type, device="cpu", save_plots=False, size=256, show_training_data=True, model=None, train_embed=None, head_layer=8, density=GaussianDensityTorch()):
    # create test dataset
    ...
    test_transform.transforms.append(transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                        std=[0.229, 0.224, 0.225]))
    test_data_eval = TestDataset(data=defect_type, transform=test_transform)
    ...
    labels = []
    embeds = []
    # print("test set: ", len(dataloader_test))
    with torch.no_grad():
        from tqdm import tqdm
        for _, sample in enumerate(tqdm(dataloader_test)):
            img = sample['image'].cuda()
            mask = sample['mask'].cuda()
            label = sample['label'].cuda()
        # for x, label in dataloader_test:
...
if __name__ == '__main__':
    ...
    # model.eval()
    valid_auc, len_valid= eval_model(args.weight, data_type, device=device,
                        save_plots=False,
                        size=256,
                        show_training_data=False,
                        model=None)
                        #train_embed=batch_embeds)
    test_auc, len_test= test_model(args.weight, data_type, device=device,
                        save_plots=False,
                        size=256,
                        show_training_data=False,
                        model=None)
